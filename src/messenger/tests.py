...
class ThreadTestCase(TestCase):
    ...
    # Añadir mensajes en un hilo
    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Muy buenas noches")
        message2 = Message.objects.create(user=self.user2, content="Muy buenas noches user2")
    ...
    #añadir otro mensaje de tercer usuario que no corresponde
    def test_add_message_from_user_not_in_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Muy buenas noches")
        message2 = Message.objects.create(user=self.user2, content="Muy buenas noches user2")
        message3 = Message.objects.create(user=self.user3, content="Soy un espía huhahahaha")
